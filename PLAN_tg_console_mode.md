...
5. [DONE] A10.3a: shared command handlers for `accounts`, `chats`, `read`, `more`, `help`, and `quit`. **high**
	...
	- Implementation boundary (2026-08-10): a new TG-owned dispatcher parses command tokens, projects valid one-shot requests onto the existing hosted adapter settings, and routes only to the accounts/chats/read adapter entry points. It owns command requests/results, help/error formatting, and an explicit in-memory read cursor context; it does not include or call MTProto, storage, session, peer, history, or model APIs. Legacy flags route through the same dispatcher and retain adapter output unchanged. One-shot `more` reports `command-more:no-previous-read`; `quit` returns a defined clean-exit result for the future REPL.
	- Invocation: put launcher flags before `-console-command`; append command options after `--`, for example `-console-command read -- user3527271 --limit 3`.
